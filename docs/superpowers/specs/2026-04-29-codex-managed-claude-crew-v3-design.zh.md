...
- 版本：V3
- 范围：在现有 V2 对抗式会话和 Claude Bridge 之上，新增一个由 Codex 管理的多 Claude worker 编排层
...
V3 不应该替换 V2，而应该在更高一层组合多个 V2 风格的 Claude bridge 会话。

```text
V3 CrewController
  使用 WorkerPool
    管理 N 个 V2 ClaudeBridgeSession
      复用 V1 ClaudeCliAdapter / WorkspaceManager / Recorder / VerificationRunner
```
...
- 复用现有 Claude bridge、verification runner、workspace manager、policy gate 和 recorder。
...
- 不做托管 SaaS 控制面。
- 初始 V3 不做模型微调，也不做自主长期运行的后台 daemon。
- 不要求每个任务都使用多个 worker；小任务可以由 Codex 选择继续走单个 V2 会话。
...
V3 应该借鉴这些概念，但不要耦合 Claude Code 内部实现。稳定集成面仍然应该是 Claude CLI 和现有 bridge 行为：
...
worker 是一个被管理的 Claude bridge 会话，带有角色化指令和权限。

初始 worker 角色：

- `explorer`：只读分析代码库、架构和风险。
- `implementer`：在隔离 worktree 或受限 workspace 里写代码。
- `reviewer`：只读评审 proposed diff 和 worker 主张。
- `verifier`：运行或建议验证命令，并报告证据。
- `competitor`：可选的备选实现者，用于高风险任务。
...
-> Explorer 阅读代码库并提交 facts/risks
-> Codex 细化 task graph
-> Implementer 在隔离 workspace 中实现
...
对于高风险或含糊的改动，Codex 可以创建两个 implementer：
...
这个模式必须使用隔离 worktree，并且最终合并决策不能互相重叠。
...
- `worktree`：worker 在隔离 git worktree 中写入。
...
| verifier | readonly or shared | no by default | 运行检查并收集证据 |
| competitor | worktree | yes, scoped | 产出备选 patch |
...
- 通过已保存 bridge session id 尝试 resume。
- 如果 resume 失败，用已有 blackboard summary 创建 replacement worker。
...
- fake Claude adapter 跑通 explorer/implementer/reviewer 流程。
- implementer patch 被记录，但不触碰主 workspace。
- 验证失败会触发 challenge。
- reviewer 拒绝会阻止 acceptance。
- worker resume 保留 bridge id 和 blackboard context。
...
### Phase 2：基于 ClaudeBridge 的 WorkerPool

在现有 Claude bridge session 之上创建 managed workers。支持 start、send、status、tail 和 stop 语义。

### Phase 3：默认 Explorer/Implementer/Reviewer 流程

实现 `crew start` 的三角色受控流程。第一版可以先让 verifier 由 Codex-run verification 承担。

### Phase 4：Worktree 写入隔离和 Merge Plan

让 implementer worker 默认使用隔离 worktree。新增 merge plan 生成和冲突检测。

### Phase 5：UI 和高级调度

在现有 UI 中展示 crew state。新增动态 worker 创建、competitor mode 和更丰富的 task graph updates。
...
- 每个 worker 都有独立 Claude bridge session 和清晰角色 prompt。
